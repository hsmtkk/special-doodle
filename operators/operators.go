...
func solve(meal_cost float64, tip_percent int32, tax_percent int32) {
	cost := CalculateTotalCost(meal_cost, tip_percent, tax_percent)
	fmt.Println(cost)
}

func CalculateTotalCost(meal_cost float64, tip_percent int32, tax_percent int32) int32 {
	tip := meal_cost * float64(tip_percent) * 0.01
	tax := meal_cost * float64(tax_percent) * 0.01
